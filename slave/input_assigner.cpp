  ...
  class EphemeralPortAssignerProcess : public InputAssignerProcess {
   public:
    ...
    Try<UDPEndpoint> register_container(
      ...
      endpoint = reader->register_container(container_id, executor_info/*, FIXME container_ip*/);
      executor_to_container[executor_info.executor_id().value()] = container_id.value();
      container_to_reader[container_id.value()] = reader;
      return endpoint;
    }
    ...
    Try<UDPEndpoint> get_statsd_endpoint(const mesos::ExecutorInfo& executor_info) {
      ...
      std::string container_id = container_iter->second;
   ...
   private:
    ...
    std::unordered_map<std::string, std::string> executor_to_container;
    ...
    std::unordered_map<std::string, port_reader_ptr_t> container_to_reader;
  };
  ...
  class PortRangeAssignerProcess : public InputAssignerProcess {
   public:
    ...
    Try<UDPEndpoint> register_container(
      ...
      Try<size_t> port = port_range_pool->take();
      if (port.isError()) {
        std::ostringstream oss;
    ...
    Nothing unregister_container(const mesos::ContainerID& container_id) {
      ...
      // Unassign this container from the reader, then delete the reader if empty (should always occur).
      if (iter->second->unregister_container_check_empty(container_id)) {
        // The reader is empty. Return the port to the pool and delete the reader using it.
        ...
        if (endpoint_to_close.isError()) {
          ...
          port_range_pool->put(endpoint_to_close->port);
        }
        container_to_reader.erase(iter);
      }
      return Nothing();
   ...
   private:
    ...
    std::shared_ptr<PortRangePool> port_range_pool;
  };
}
