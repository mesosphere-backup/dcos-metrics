...
stats::SinglePortStrategy::SinglePortStrategy(
...
stats::EphemeralPortStrategy::EphemeralPortStrategy(std::shared_ptr<IORunner> io_runner)
  : io_runner(io_runner) { }
stats::EphemeralPortStrategy::~EphemeralPortStrategy() { }

Try<stats::UDPEndpoint> stats::EphemeralPortStrategy::register_container(
    const mesos::ContainerID& container_id, const mesos::ExecutorInfo& executor_info) {
...
void stats::EphemeralPortStrategy::insert_container(
    const mesos::ContainerID& container_id,
    const mesos::ExecutorInfo& executor_info,
    const UDPEndpoint& endpoint) {
...
void stats::EphemeralPortStrategy::unregister_container(
    const mesos::ContainerID& container_id) {
  auto iter = container_to_reader.find(container_id);
  if (iter == container_to_reader.end()) {
...
stats::PortRangeStrategy::PortRangeStrategy(
    std::shared_ptr<IORunner> io_runner, const mesos::Parameters& parameters)
  : io_runner(io_runner) {
...
Try<stats::UDPEndpoint> stats::PortRangeStrategy::register_container(
    const mesos::ContainerID& container_id, const mesos::ExecutorInfo& executor_info) {
  // Reuse existing reader if available.
  // This isn't expected to happen in practice, but just in case..
  auto iter = container_to_reader.find(container_id);
...
void stats::PortRangeStrategy::insert_container(
    const mesos::ContainerID& container_id,
    const mesos::ExecutorInfo& executor_info,
    const UDPEndpoint& endpoint) {
...
void stats::PortRangeStrategy::unregister_container(
    const mesos::ContainerID& container_id) {
  auto iter = container_to_reader.find(container_id);
  if (iter == container_to_reader.end()) {
