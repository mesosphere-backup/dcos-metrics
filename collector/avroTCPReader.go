...
func RunAvroTCPReader(recordsChan chan<- *AvroDatum, stats chan<- events.StatsEvent) {
	addr, err := net.ResolveTCPAddr("tcp", *listenEndpointFlag)
	if err != nil {
		stats <- events.MakeEvent(events.TCPResolveFailed)
		log.Fatalf("Failed to parse TCP endpoint '%s': %s", *listenEndpointFlag, err)
	}
	sock, err := net.ListenTCP("tcp", addr)
	if err != nil {
		stats <- events.MakeEvent(events.TCPListenFailed)
	...
	for {
		...
		if err != nil {
			stats <- events.MakeEvent(events.TCPAcceptFailed)
			log.Printf("Failed to accept connection on TCP endpoint '%s': %s\n",
				*listenEndpointFlag, err)
			continue
		}
		stats <- events.MakeEvent(events.TCPSessionOpened)
...
// This function should be run as a gofunc.
func handleConnection(conn *net.TCPConn, recordsChan chan<- *AvroDatum, stats chan<- events.StatsEvent) {
	conn.SetKeepAlive(true)
	defer func() {
		stats <- events.MakeEvent(events.TCPSessionClosed)
		conn.Close()
	}()
	...
	if err != nil {
		stats <- events.MakeEvent(events.AvroReaderOpenFailed)
		log.Println("Failed to create avro reader:", err)
		return // close connection
	}
	defer func() {
		if err := avroReader.Close(); err != nil {
			stats <- events.MakeEvent(events.AvroReaderCloseFailed)
	...
	for {
		...
		if !ok {
			stats <- events.MakeEvent(events.RecordBadTopic)
		}
		...
		stats <- events.MakeEventSuff(events.AvroRecordIn, topic)
		stats <- events.MakeEventSuffCount(events.AvroBytesIn, topic, int(approxBytesRead))
		...
		if reader.inputBytes > *inputLimitAmountKBytesFlag*1024 {
			...
			stats <- events.MakeEventSuff(events.AvroRecordInThrottled, topic)
			stats <- events.MakeEventSuffCount(events.AvroBytesInThrottled, topic, int(approxBytesRead))
			continue
		}
