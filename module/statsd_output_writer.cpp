...
metrics::StatsdOutputWriter::StatsdOutputWriter(std::shared_ptr<boost::asio::io_service> io_service,
    const mesos::Parameters& parameters,
    size_t chunk_timeout_ms_for_tests/*=1000*/,
    size_t resolve_period_ms_for_tests/*=0*/)
  : send_host(params::get_str(parameters, params::OUTPUT_STATSD_HOST, params::OUTPUT_STATSD_HOST_DEFAULT)),
    send_port(params::get_uint(parameters, params::OUTPUT_STATSD_PORT, params::OUTPUT_STATSD_PORT_DEFAULT)),
...
metrics::StatsdOutputWriter::udp_resolver_t::iterator metrics::StatsdOutputWriter::resolve(
    boost::system::error_code& ec) {
  udp_resolver_t resolver(*io_service);
  return resolver.resolve(udp_resolver_t::query(send_host, ""), ec);
}

void metrics::StatsdOutputWriter::shutdown() {
...
void metrics::StatsdOutputWriter::start_dest_resolve_timer() {
...
void metrics::StatsdOutputWriter::dest_resolve_cb(boost::system::error_code ec) {
  if (ec) {
...
void metrics::StatsdOutputWriter::send_raw_bytes(const char* bytes, size_t size) {
  if (size == 0) {
