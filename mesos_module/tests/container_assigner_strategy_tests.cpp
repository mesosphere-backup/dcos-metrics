...
namespace {
  ...
  inline Try<metrics::UDPEndpoint> try_endpoint(const std::string& host, size_t port) {
    return metrics::UDPEndpoint(host, port);
  }
...
TEST_F(ContainerAssignerStategyTests, single_port_bad_port) {
  ...
  EXPECT_DETH(new metrics::SinglePortStrategy(mock_runner, params),
...
TEST_F(ContainerAssignerStategyTests, single_port) {
  ...
  metrics::SinglePortStrategy strategy(mock_runner, params);
  ...
  EXPECT_CALL(*mock_reader1, open()).WillOnce(Return(Try<metrics::UDPEndpoint>(Error("test fail"))));
  ...
  Try<metrics::UDPEndpoint> endpt = strategy.register_container(ci1, ei1);
  ...
  EXPECT_CALL(*mock_reader1, endpoint()).WillOnce(Return(
          Try<metrics::UDPEndpoint>(Error("test fail"))));
  ...
  EXPECT_CALL(*mock_reader1, endpoint()).WillOnce(
      Return(Try<metrics::UDPEndpoint>(Error("test fail"))));
  EXPECT_CALL(*mock_reader1, register_container(ContainerIdMatch(ci3), ExecInfoMatch(ei3)));
  strategy.insert_container(ci3, ei3, metrics::UDPEndpoint(host1, port1));
...
TEST_F(ContainerAssignerStategyTests, ephemeral_port) {
  metrics::EphemeralPortStrategy strategy(mock_runner);
  mesos::ContainerID ci1 = container_id("cid1"),
    ci2 = container_id("cid2"),
    ci3 = container_id("cid3");
  mesos::ExecutorInfo ei1 = exec_info("fid1", "eid1"),
  ...
  EXPECT_CALL(*mock_reader1, endpoint())
    .WillOnce(Return(Try<metrics::UDPEndpoint>(Error("ignored"))));
  ...
  EXPECT_CALL(*mock_reader1, open()).WillOnce(Return(Try<metrics::UDPEndpoint>(Error("test fail"))));
  strategy.insert_container(ci3, ei3, metrics::UDPEndpoint("ignored", port1));
  ...
  strategy.insert_container(ci3, ei3, metrics::UDPEndpoint(host2, port2));
...
TEST_F(ContainerAssignerStategyTests, port_range_bad_ports) {
  ...
  param->set_key(metrics::params::OUTPUT_STATSD_HOST);
  param->set_value("host1");
  EXPECT_DETH(new metrics::PortRangeStrategy(mock_runner, params),
      "Invalid listen_port_start config value: 0");

  // Test listen_port_start (end unset)
  param = params.add_parameter();
  param->set_key(metrics::params::LISTEN_PORT_START);
  param->set_value("0");
  EXPECT_DETH(new metrics::PortRangeStrategy(mock_runner, params),
      "Invalid listen_port_start config value: 0");
  ...
  param = params.add_parameter();
  param->set_key(metrics::params::LISTEN_PORT_END);
  param->set_value("0");
  EXPECT_DETH(new metrics::PortRangeStrategy(mock_runner, params),
      "Invalid listen_port_end config value: 0");
...
TEST_F(ContainerAssignerStategyTests, port_range) {
  ...
  metrics::PortRangeStrategy strategy(mock_runner, params);
  mesos::ContainerID ci1 = container_id("cid1"),
    ci2 = container_id("cid2"),
    ci3 = container_id("cid3");
  mesos::ExecutorInfo ei1 = exec_info("fid1", "eid1"),
  ...
  strategy.insert_container(ci3, ei3, metrics::UDPEndpoint("ignored", port3));
  ...
  EXPECT_CALL(*mock_runner, create_container_reader(port3)).WillOnce(Return(mock_reader2));
  EXPECT_CALL(*mock_reader2, open()).WillOnce(Return(Try<metrics::UDPEndpoint>(Error("test fail"))));
  strategy.insert_container(ci3, ei3, metrics::UDPEndpoint("ignored", port3));
